...
import type { RequestOptions, RequestResult } from '@vue-admin/types'
...
import {
  ...
  deepMerge,
  setObjToUrlParams,
} from '@vue-admin/utils'
import { RequestEnum, ResultEnum, ContentTypeEnum } from '@vue-admin/tokens'
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (
    ...
    const { isTransformResponse, isReturnNativeResponse, replaceDataResponse } =
      options
    ...
    for (let key in replaceDataResponse) {
      if (
        !!replaceDataResponse[key] &&
        !data.hasOwnProperty(key)
      ) {
        data[key] = data[replaceDataResponse[key]]
        delete data[replaceDataResponse[key]]
      }
    }
    console.info('响应信息', data)

    //  这里 code，result，message为 后台统一的字段，根据需要在replaceDataResponse替换自己返回的字段
